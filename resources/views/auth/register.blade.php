<x-layout>
    <div class="bg-white rounded-lg shadow-md w-full md:max-w-xl mx-auto mt-12 p-8 py-12">
        ...
        <form method="POST" action="{{ route('register.store') }}">
            @csrf
            <x-inputs.text id="name" name="name" placeholder="User Full Name" />
            <x-inputs.text type="email" id="email" name="email" placeholder="User email" />
            <x-inputs.text type="password" id="password" name="password" placeholder="User Password" />
